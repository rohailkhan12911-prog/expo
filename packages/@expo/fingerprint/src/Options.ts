...
export async function normalizeOptionsAsync(
  ...
  return {
    ...
    // Options from config
    ...config,
    // Explicit options (an explicit `sourceSkips` replaces the preset's)
    ...
    enableReactImportsPatcher:
      ...
      false,
    // Preset-derived settings that aren't user-tunable directly.
    packageMode: preset.packageMode,
    configPluginTrace: preset.configPluginTrace,
