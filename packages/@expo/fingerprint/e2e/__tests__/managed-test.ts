...
function normalizeAutolinkingSourceVersionsForSnapshot<
  T extends HashSource | FingerprintSource
>(source: T): T {
  ...
  const normalizedSource = {
    ...
    contents: JSON.stringify(
      JSON.parse(source.contents.toString()),
      (key, value) =>
  ...
  return (
    "hash" in normalizedSource
      ? { ...normalizedSource, hash: "*" }
      : normalizedSource
  ) as T;
}

function normalizeAutolinkingVersionsForSnapshot(
  sources: HashSources
): HashSources;
function normalizeAutolinkingVersionsForSnapshot(
...
function normalizeAutolinkingVersionsForSnapshot(
  sourcesOrDiff: HashSources | FingerprintDiff
) {
  return sourcesOrDiff.map((item) => {
    ...
    switch (item.op) {
      case "added":
        return {
          ...item,
          addedSource: normalizeAutolinkingSourceVersionsForSnapshot(
            item.addedSource
          ),
        };
      case "removed":
        return {
          ...item,
          removedSource: normalizeAutolinkingSourceVersionsForSnapshot(
            item.removedSource
          ),
        };
      case "changed":
        return {
          ...item,
          beforeSource: normalizeAutolinkingSourceVersionsForSnapshot(
            item.beforeSource
          ),
          afterSource: normalizeAutolinkingSourceVersionsForSnapshot(
            item.afterSource
          ),
        };
    }
...
describe("managed project test", () => {
  ...
  beforeAll(async () => {
    ...
    await spawnAsync(
      ...
      [
        "create-expo-app",
        "-t",
        `blank@${E2E_TEMPLATE_SDK_VERSION}`,
        projectName,
      ],
      {
        stdio: "inherit",
    ...
    originalConfig = JSON.parse(
      await fs.readFile(path.join(projectRoot, "app.json"), "utf8")
    );
  });
  ...
  it("diffFingerprintChangesAsync - should return diff after adding native library", async () => {
    ...
    await spawnAsync(
      "bun",
      ["install", "--save", "@react-native-community/netinfo@12.0.1"],
      {
        stdio: "ignore",
        cwd: projectRoot,
      }
    ...
    expect(normalizeAutolinkingVersionsForSnapshot(diff))
  ...
  it("should have same hash even if google service file path is different", async () => {
    ...
    const tmpDir = require("temp-dir");
    const googleServicesPathNew = path.join(
      tmpDir,
    ...
    try {
      ...
      await fs.cp(googleServicesPath, googleServicesPathNew, {
        recursive: true,
      });
      ...
      await fs.rm(path.dirname(googleServicesPathNew), {
        recursive: true,
        force: true,
      });
    }
