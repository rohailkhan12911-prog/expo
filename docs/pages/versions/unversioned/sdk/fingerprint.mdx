...
A preset is a named bundle of settings that trades some fidelity for fewer false positives (a different fingerprint from a change that doesn't affect the native build). Set it with the `preset` field in **fingerprint.config.js** or the `--preset` CLI flag. An explicitly provided `sourceSkips` overrides the preset's source skips.

| Preset       | Description                                                                                                                                                                                                             |
| ------------ | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `strict`     | Highest fidelity. Hashes autolinked packages and config plugins by their full native directories.                                                                                                                          |
| `balanced`   | **The default.** Ignores app version and string runtime version churn, hashes autolinked packages by their `package.json` version, and collapses `node_modules` config plugins to their package version. Best for most apps. |
| `relaxed`    | For building multiple variants from one native project. In addition to `balanced`, ignores app names, Android package, iOS bundle identifier, schemes, and assets. Still hashes the config-plugins list, so adding a plugin changes the fingerprint. |
